...
class Agent(nn.Module):
    ...
    def update(
        ...
        if train_world_model:
            ...
            logger.log("WorldModel/frozen", 1)
    ...
    @torch.no_grad()
    def log_world_model_metrics(self, replay_buffer, batch_size, batch_length, logger, prefix="eval"):
        if len(replay_buffer) < batch_length:
            return
        self.world_model.eval()
        state, obs, action, reward, termination = replay_buffer.sample(batch_size, batch_length)
        self.world_model.log_eval_metrics(state, obs, action, reward, termination, logger, prefix=prefix)
