...
from utils.spatial_regularization import (
    SpatialRegularizationConfig,
    compute_spatial_regularization,
    log_spatial_regularization,
)
...
class WorldModel(nn.Module):
    def __init__(
        ...
        self.spatial_regu = spatial_regu or SpatialRegularizationConfig()
    ...
    def encode_obs(self, state, obs, sample_mode="random_sample"):
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            ...
            state_sample = self.straight_through_gradient(state_post_logits, sample_mode=sample_mode)
            ...
            obs_sample = self.straight_through_gradient(obs_post_logits, sample_mode=sample_mode)
    ...
    def predict_next(self, last_state_flattened_sample, last_obs_flattened_sample, action, log_video=True, sample_mode="random_sample"):
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            ...
            state_prior_sample = self.straight_through_gradient(state_prior_logits, sample_mode=sample_mode)
            state_flattened_prior_sample = einops.rearrange(state_prior_sample, "B L Obj K C -> B L Obj (K C)")
            obs_prior_sample = self.straight_through_gradient(obs_prior_logits, sample_mode=sample_mode)
            obs_flattened_prior_sample = einops.rearrange(obs_prior_sample, "B L K C -> B L (K C)")
            if log_video:
                obs_hat = self.visual_decoder(obs_flattened_prior_sample)
            else:
                obs_hat = None
        ...
        return obs_hat, reward_hat, termination_hat, state_flattened_prior_sample, obs_flattened_prior_sample, dist_feat
    ...
    def update(self, state, obs, action, reward, termination, logger=None, masks=None):
        self.train()
        ...
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            ...
            spatial_regu_loss, spatial_regu_metrics = compute_spatial_regularization(
                obs_hat,
                obs,
                masks,
                self.spatial_regu,
                return_metrics=logger is not None,
            )
            ...
            abs_total = (
                reconstruction_loss.detach().abs()
                + reward_loss.detach().abs()
                + termination_loss.detach().abs()
                + spatial_regu_loss.detach().abs()
                + (0.5 * dynamics_loss).detach().abs()
                + (0.1 * representation_loss).detach().abs()
            ).clamp_min(1e-8)
            if spatial_regu_metrics:
                spatial_regu_metrics["lossfrac/spatial_regu"] = spatial_regu_loss.detach().abs() / abs_total
                for idx in (1, 2, 3):
                    key = f"mask{idx}"
                    scaled_key = f"loss_scaled/spatial_regu/{key}"
                    if scaled_key in spatial_regu_metrics:
                        spatial_regu_metrics[f"lossfrac/spatial_regu/{key}"] = (
                            spatial_regu_metrics[scaled_key].detach().abs() / abs_total
                        )
        ...
        if logger is not None:
            ...
            logger.log("loss_unscaled/state", state_reconstruction_loss.item())
            logger.log("loss_scaled/state", state_reconstruction_loss.item())
            logger.log("lossfrac/state", (state_reconstruction_loss.detach().abs() / abs_total).item())
            logger.log("loss_unscaled/image", obs_reconstruction_loss.item())
            logger.log("loss_scaled/image", obs_reconstruction_loss.item())
            logger.log("lossfrac/image", (obs_reconstruction_loss.detach().abs() / abs_total).item())
            logger.log("loss_unscaled/reconstruction", reconstruction_loss.item())
            logger.log("loss_scaled/reconstruction", reconstruction_loss.item())
            logger.log("lossfrac/reconstruction", (reconstruction_loss.detach().abs() / abs_total).item())
            log_spatial_regularization(logger, spatial_regu_metrics)
            ...
            logger.log("loss_unscaled/reward", reward_loss.item())
            logger.log("loss_scaled/reward", reward_loss.item())
            logger.log("lossfrac/reward", (reward_loss.detach().abs() / abs_total).item())
            logger.log("loss_unscaled/termination", termination_loss.item())
            logger.log("loss_scaled/termination", termination_loss.item())
            logger.log("lossfrac/termination", (termination_loss.detach().abs() / abs_total).item())
            logger.log("loss_unscaled/dynamics", dynamics_loss.item())
            logger.log("loss_scaled/dynamics", (0.5 * dynamics_loss).item())
            logger.log("lossfrac/dynamics", ((0.5 * dynamics_loss).detach().abs() / abs_total).item())
            logger.log("loss_unscaled/representation", representation_loss.item())
            logger.log("loss_scaled/representation", (0.1 * representation_loss).item())
            logger.log("lossfrac/representation", ((0.1 * representation_loss).detach().abs() / abs_total).item())
            logger.log("loss_scaled/total", total_loss.item())

    @torch.no_grad()
    def log_eval_metrics(self, state, obs, action, reward, termination, logger, prefix="eval", masks=None):
        self.eval()
        ...
        logger.log(f"{prefix}/WorldModel/reconstruction_loss", reconstruction_loss.item())
        logger.log(f"{prefix}/WorldModel/state_reconstruction_loss", state_reconstruction_loss.item())
        logger.log(f"{prefix}/WorldModel/obs_reconstruction_loss", obs_reconstruction_loss.item())
        logger.log(f"{prefix}/loss_unscaled/state", state_reconstruction_loss.item())
        logger.log(f"{prefix}/loss_scaled/state", state_reconstruction_loss.item())
        logger.log(f"{prefix}/lossfrac/state", (state_reconstruction_loss.detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_unscaled/image", obs_reconstruction_loss.item())
        logger.log(f"{prefix}/loss_scaled/image", obs_reconstruction_loss.item())
        logger.log(f"{prefix}/lossfrac/image", (obs_reconstruction_loss.detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_unscaled/reconstruction", reconstruction_loss.item())
        logger.log(f"{prefix}/loss_scaled/reconstruction", reconstruction_loss.item())
        logger.log(f"{prefix}/lossfrac/reconstruction", (reconstruction_loss.detach().abs() / abs_total).item())
        logger.log(f"{prefix}/WorldModel/reward_loss", reward_loss.item())
        logger.log(f"{prefix}/WorldModel/termination_loss", termination_loss.item())
        logger.log(f"{prefix}/WorldModel/dynamics_loss", dynamics_loss.item())
        logger.log(f"{prefix}/WorldModel/dynamics_real_kl_div", dynamics_real_kl_div.item())
        logger.log(f"{prefix}/WorldModel/total_loss", total_loss.item())
        logger.log(f"{prefix}/loss_unscaled/reward", reward_loss.item())
        logger.log(f"{prefix}/loss_scaled/reward", reward_loss.item())
        logger.log(f"{prefix}/lossfrac/reward", (reward_loss.detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_unscaled/termination", termination_loss.item())
        logger.log(f"{prefix}/loss_scaled/termination", termination_loss.item())
        logger.log(f"{prefix}/lossfrac/termination", (termination_loss.detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_unscaled/dynamics", dynamics_loss.item())
        logger.log(f"{prefix}/loss_scaled/dynamics", (0.5 * dynamics_loss).item())
        logger.log(f"{prefix}/lossfrac/dynamics", ((0.5 * dynamics_loss).detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_unscaled/representation", representation_loss.item())
        logger.log(f"{prefix}/loss_scaled/representation", (0.1 * representation_loss).item())
        logger.log(f"{prefix}/lossfrac/representation", ((0.1 * representation_loss).detach().abs() / abs_total).item())
        logger.log(f"{prefix}/loss_scaled/total", total_loss.item())
        for key, value in spatial_regu_metrics.items():
