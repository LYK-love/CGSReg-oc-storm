...
def get_causal_mask(seq):
    ...
    return get_causal_mask_with_batch_length(batch_length, seq.device)


def get_causal_mask_with_batch_length(batch_length, device):
    """For masking out the subsequent info."""
    key = (str(device), batch_length)
    if key not in _CAUSAL_MASK_CACHE:
        _CAUSAL_MASK_CACHE[key] = (
            1 - torch.triu(torch.ones((1, batch_length, batch_length), device=device), diagonal=1)
        ).bool()
...
def get_vector_mask(batch_length, device):
    key = (str(device), batch_length)
    if key not in _VECTOR_MASK_CACHE:
        _VECTOR_MASK_CACHE[key] = torch.ones((1, 1, batch_length), device=device).bool()
    return _VECTOR_MASK_CACHE[key]
