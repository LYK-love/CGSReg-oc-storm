...
class ActorCritic(nn.Module):
    ...
    def update(self, policy_input, action, old_log_prob, reward, termination, logger=None):
        ...
        if logger is not None:
            rollout_return = reward.sum(dim=1)
            rollout_length = (1 - termination).sum(dim=1)
            logger.log("ActorCritic/wm_rollout_reward_mean", reward.mean().item())
            logger.log("ActorCritic/wm_rollout_return_mean", rollout_return.mean().item())
            logger.log("ActorCritic/wm_rollout_return_std", rollout_return.std(unbiased=False).item())
            logger.log("ActorCritic/wm_rollout_return_min", rollout_return.min().item())
            logger.log("ActorCritic/wm_rollout_return_max", rollout_return.max().item())
            logger.log("ActorCritic/wm_rollout_length_mean", rollout_length.float().mean().item())
            logger.log("ActorCritic/wm_lambda_return_mean", lambda_return.mean().item())
            logger.log("ActorCritic/wm_lambda_return_std", lambda_return.std(unbiased=False).item())
