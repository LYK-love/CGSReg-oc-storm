...
class WorldModel(nn.Module):
    ...
    def predict_next(self, last_flattened_sample, action, log_video=True, sample_mode="random_sample"):
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            dist_feat = self.storm_transformer.forward_with_kv_cache(last_flattened_sample, action)
            prior_logits = self.dist_head.forward_prior(dist_feat)

            # decoding
            prior_sample = self.straight_through_gradient(prior_logits, sample_mode=sample_mode)
            prior_flattened_sample = self.flatten_sample(prior_sample)
            if log_video:
                obs_hat = self.state_decoder(prior_flattened_sample)
            else:
                obs_hat = None
    ...
    def update(self, state, action, reward, termination, logger=None, masks=None):
        self.train()
        batch_size, batch_length = state.shape[:2]

        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            state_image, state_masks = split_image_and_masks(state, self.spatial_regu.image_channels)
            if masks is None:
                masks = state_masks
            ...
            image_reconstruction_loss = self.mse_loss_func(
                state_hat[:, :, : self.spatial_regu.image_channels],
                state_image,
            )
        ...
        if logger is not None:
            ...
            logger.log("loss_unscaled/image", image_reconstruction_loss.item())
            logger.log("loss_scaled/image", image_reconstruction_loss.item())
            logger.log("lossfrac/image", (image_reconstruction_loss.detach().abs() / abs_total).item())
    ...
    @torch.no_grad()
    def log_eval_metrics(self, state, action, reward, termination, logger, prefix="eval", masks=None):
        self.eval()
        batch_size, batch_length = state.shape[:2]
        with torch.autocast(device_type="cuda", dtype=self.amp_tensor_dtype, enabled=self.use_amp):
            state_image, state_masks = split_image_and_masks(state, self.spatial_regu.image_channels)
        ...
        logger.log(f"{prefix}/loss_unscaled/image", image_reconstruction_loss.item())
        logger.log(f"{prefix}/loss_scaled/image", image_reconstruction_loss.item())
        logger.log(f"{prefix}/lossfrac/image", (image_reconstruction_loss.detach().abs() / abs_total).item())
