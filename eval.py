...
if __name__ == "__main__":
    ...
    parser.add_argument("--ram", nargs="?", const=True, default=False, type=optional_str2bool)
    ...
    if args.save_dataset:
        dataset_dir = args.dataset_dir or f"runs/{args.run_name}/dataset/eval_from_eval"
        meta = replay_buffer.export_offline_dataset(dataset_dir, include_ram=args.ram)
        dataset_path = Path(dataset_dir)
        if dataset_path.name in {"eval", "eval_from_eval"}:
            manifest = {
                "format": "oc_storm_offline_dataset_v1",
                "source": f"eval.py run {args.run_name}",
                "ram": bool(args.ram),
                "splits": {dataset_path.name: meta},
            }
            (dataset_path.parent / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
        print(
            colorama.Fore.GREEN
            + f"Exported eval dataset {meta['num_episodes']} episodes / {meta['num_steps']} steps to {dataset_dir}"
