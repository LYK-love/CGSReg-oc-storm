import colorama
import os
from typing import Tuple

# Envs - Atari
from gymnasium.core import Env
from gymnasium import spaces
from envs.atari.build_env import build_single_atari_env, ActionSpace

# Feature Extractor - Visual
from feature_extractor.visual.build_feature_extractor import ResizeObservation

# Buffer
from utils.replay_buffer import VisualReplayBuffer

# Agent - Visual input (STORM)
from agents.visual.agent import Agent
from utils.spatial_regularization import SpatialRegularizationConfig

# Params
from dataclasses import dataclass, field


@dataclass()
class WorldModelParams:
    transformer_max_length: int = 64
    transformer_hidden_dim: int = 256
    transformer_num_layers: int = 2
    transformer_num_heads: int = 4


@dataclass()
class PolicyParams:
    num_layers: int = 3
    hidden_dim: int = 512
    gamma: float = 0.985
    lambd: float = 0.95
    entropy_coef: float = 1e-3


@dataclass()
class Params:
    num_objects: int = -1  # will be set in build()
    frame_skip: int = 4

    object_feature_dim: int = 2048

    buffer_max_length: int = int(1e5)
    buffer_warm_up: int = 1024

    latent_width: int = 16
    world_model: WorldModelParams = field(default_factory=WorldModelParams)
    policy: PolicyParams = field(default_factory=PolicyParams)

    max_sample_steps: int = int(1e5) + 20
    min_train_ratio: float = 1.0  # train_steps/sample_steps, has nothing to do with batch_size, etc.
    max_train_ratio: float = 1.0

    batch_size: int = 32
    batch_length: int = 32

    imagine_batch_size: int = 512
    imagine_context_length: int = 4
    imagine_batch_length: int = 16

    eval_context_length: int = 8

    save_every_steps: int = int(2e4)
    spatial_regu: SpatialRegularizationConfig = field(default_factory=SpatialRegularizationConfig)


MODEL_SIZE_PRESETS = {
    "base": {},
    "size200m": {
        "transformer_hidden_dim": 1024,
        "transformer_num_layers": 8,
        "transformer_num_heads": 8,
        "policy_hidden_dim": 1024,
        "policy_num_layers": 4,
    },
    "size400m": {
        "transformer_hidden_dim": 1536,
        "transformer_num_layers": 12,
        "transformer_num_heads": 12,
        "policy_hidden_dim": 1536,
        "policy_num_layers": 4,
    },
    "size800m": {
        "transformer_hidden_dim": 2048,
        "transformer_num_layers": 16,
        "transformer_num_heads": 16,
        "policy_hidden_dim": 2048,
        "policy_num_layers": 5,
    },
}


def apply_model_size(params: Params, size_config: str | None = None) -> Params:
    size_config = size_config or os.environ.get("STORM_SIZE_CONFIG", "base")
    if size_config in {"", "default"}:
        size_config = "base"
    if size_config not in MODEL_SIZE_PRESETS:
        raise ValueError(
            f"Unknown STORM size config {size_config!r}. "
            f"Expected one of {sorted(MODEL_SIZE_PRESETS)}."
        )
    preset = MODEL_SIZE_PRESETS[size_config]
    if "transformer_hidden_dim" in preset:
        params.world_model.transformer_hidden_dim = preset["transformer_hidden_dim"]
    if "transformer_num_layers" in preset:
        params.world_model.transformer_num_layers = preset["transformer_num_layers"]
    if "transformer_num_heads" in preset:
        params.world_model.transformer_num_heads = preset["transformer_num_heads"]
    if "policy_hidden_dim" in preset:
        params.policy.hidden_dim = preset["policy_hidden_dim"]
    if "policy_num_layers" in preset:
        params.policy.num_layers = preset["policy_num_layers"]
    params.size_config = size_config
    return params


def make_params(size_config: str | None = None) -> Params:
    return apply_model_size(Params(), size_config=size_config)


def build(env_name, seed) -> Tuple[Params, Env, ActionSpace, ResizeObservation, VisualReplayBuffer, Agent]:
    params = make_params()

    # Build Env >>>
    env, action_space = build_single_atari_env(env_name, seed)
    print("Possible actions: " + colorama.Fore.YELLOW + f"{action_space.choices_per_dim}" + colorama.Style.RESET_ALL)
    # <<< Build Env

    # Build Feature Extractor >>>
    feature_extractor = ResizeObservation(state_resolution=(64, 64))
    # <<< Build Feature Extractor

    # Buffer
    replay_buffer = VisualReplayBuffer(
        obs_shape=(3, 64, 64),
        action_dim=1,
        num_envs=1,
        max_length=params.buffer_max_length,
        warmup_length=params.buffer_warm_up,
        store_on_gpu=True,
    )

    # Agent
    agent = Agent(
        input_channels=3,
        action_dims=[action_space.choices_per_dim],
        num_objects=params.num_objects,
        latent_width=params.latent_width,
        transformer_max_length=params.world_model.transformer_max_length,
        transformer_hidden_dim=params.world_model.transformer_hidden_dim,
        transformer_num_layers=params.world_model.transformer_num_layers,
        transformer_num_heads=params.world_model.transformer_num_heads,
        policy_num_layers=params.policy.num_layers,
        policy_hidden_dim=params.policy.hidden_dim,
        gamma=params.policy.gamma,
        lambd=params.policy.lambd,
        entropy_coef=params.policy.entropy_coef,
        spatial_regu=params.spatial_regu,
    ).cuda()

    return params, env, action_space, feature_extractor, replay_buffer, agent


def build_offline(env_name, seed) -> Tuple[Params, Agent]:
    params = make_params()
    env, action_space = build_single_atari_env(env_name, seed)
    if hasattr(env, "close"):
        env.close()

    agent = Agent(
        input_channels=3,
        action_dims=[action_space.choices_per_dim],
        num_objects=params.num_objects,
        latent_width=params.latent_width,
        transformer_max_length=params.world_model.transformer_max_length,
        transformer_hidden_dim=params.world_model.transformer_hidden_dim,
        transformer_num_layers=params.world_model.transformer_num_layers,
        transformer_num_heads=params.world_model.transformer_num_heads,
        policy_num_layers=params.policy.num_layers,
        policy_hidden_dim=params.policy.hidden_dim,
        gamma=params.policy.gamma,
        lambd=params.policy.lambd,
        entropy_coef=params.policy.entropy_coef,
        spatial_regu=params.spatial_regu,
    ).cuda()
    return params, agent
